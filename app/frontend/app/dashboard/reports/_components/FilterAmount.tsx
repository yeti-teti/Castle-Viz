...
function FilterAmount() {
  ...
  const [minAmount, maxAmount] = React.useMemo(() => {
    if (transactions.length === 0) {
      return [0, 10000] // Default range when no data
    }
    const amounts = transactions.map((t) => t.amount / 100) // Convert from cents to dollars
  ...
  const distributionData = React.useMemo(() => {
    if (transactions.length === 0) {
      return Array(30).fill({ height: 0, isInRange: false });
    }
    ...
    transactions.forEach((t) => {
      const amount = t.amount / 100; // Convert from cents to dollars
  ...
  }, [transactions, minAmount, maxAmount, localMin, localMax])

  if (loading) {
    return (
      <div>
        <Label htmlFor="amount-filter" className="font-medium text-black-900 dark:text-black-50">
          Transaction Amount
        </Label>
        <div className="mt-2 h-10 w-full md:w-36 animate-pulse bg-gray-200 rounded" />
      </div>
    )
  }
